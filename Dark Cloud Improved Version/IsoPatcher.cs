...
namespace Dark_Cloud_Improved_Version
{
    ...
    internal static class IsoPatcher
    {
        ...
        static void PatchNativeCameraPostPass(FileStream fs, Func<uint, long> ElfOff)
        {
            ...
            // ===== CAMERA TUNABLES (tools/pullin2.s) — injected into the template's constant slots below (PutVal =
            //       single `lui`, integer/.25-step values; PutEase = `lui`+`ori`, any float); auto-located indices. =====
            // ARCHITECTURE (post pull-in removal): dist target = BASE_DIST always (NO wall-ray pull-in — no snap when
            //   walking behind a wall; occlusion response = future autorotate). Height target = REST_H + stick on free
            //   frames, HELD at current on contact frames (flag @0x14C21C). Ceiling probe DUCKS it, ground probe floors
            //   it. The SWEPT-SLIDE (E_prev @0x14C210 → eased target, +SLIDE_MARGIN proximity reach) resolves contact
            //   on the authored-normal side (weighted: SLIDE_BIAS angle share, SLIDE_FRICTION contact drag). The
            //   GEOMETRIC CLIMB: on contact, h = REST_H + CLIMB_K·(BASE−d)² — a gentle-onset curve (zero slope at touch),
            //   purely geometric so a wall traversal reads as a BELL; θ/dist reacquisition slides toward rest at
            //   SLIDE_GAIN (Δθ share stick-overridden); a VERIFY cast + second resolution handle corner seams
            //   (regs SPILLED across it — CheckHit tramples caller-saved FP). Post-slide h clamps: max.s/min.s. ⚠ EE gotchas: c.OLT.s/sqrt.s/max.s/min.s are .word-encoded; nop after every
            //   mtc1 and FP compare. [[native-camera-functions]]
            ...
            uint[] pullIn =
            {
                0x27BDFF60, 0xAFBF0050, 0x3C0101D2, 0x8C239678, 0x1060020A, 0x00000000,
                0xAFA30058, 0xC46002C0, 0xE7A00020, 0xC46002C4, 0xE7A00024, 0xC46002C8,
                0xE7A00028, 0xC46C02DC, 0x0C047628, 0x00000000, 0xE7A00060, 0x8FA30058,
                0xC46C02DC, 0x0C0475AC, 0x00000000, 0xE7A00064, 0x8FA30058, 0xC46102D0,
                0xC7A20060, 0x46011082, 0xC7A30020, 0x46021900, 0xC7A20064, 0x46011082,
                0xC7A30028, 0x46021940, 0xC7A60024, 0xC46702D4, 0x460731C0, 0xE7A40020,
                0xE7A70024, 0xE7A50028, 0xAFA0002C, 0xE7A40030, 0x3C0842C8, 0x44880800,
                0x00000000, 0x46013800, 0xE7A00034, 0xE7A50038, 0xAFA0003C, 0x02A02021,
                ...
                0x00000000, 0xC7A00044, 0xE7A0006C, 0x10000005, 0x00000000, 0x3C084800,
                0x44880000, 0x00000000, 0xE7A0006C, 0xC7A00020, 0xE7A00030, 0xC7A00024,
                0x3C0843FA, 0x44880800, 0x00000000, 0x46010001, 0xE7A00034, 0xC7A00028,
                0xE7A00038, 0x02A02021, 0x03C02821, 0x27A60020, 0x27A70030, 0x27A80040,
                0xAFA80010, 0x24090001, 0xAFA90014, 0xAFA00018, 0x0C052754, 0x00000000,
                0x8FA30058, 0x04400005, 0x00000000, 0xC7A00044, 0xE7A0005C, 0x10000002,
                0x00000000, 0xAFA0005C, 0x0C05A67C, 0x00000000, 0x440A0000, 0x00000000,
                0x000A5040, 0x3C0B7D00, 0x016A502B, 0xAFAA0084, 0x0C05A68C, 0x00000000,
                ...
                0x46010002, 0x10000002, 0x00000000, 0x44800000, 0x3C0B0014, 0x356BC20C,
                ...
                0x44880000, 0x00000000, 0x3C084188, 0x44882800, 0x00000000, 0xC7A80068,
                0x46082940, 0xC7A6006C, 0xC7A70024, 0x46073181, 0x3C084160, 0x44883800,
                0x00000000, 0x46073181, 0xE7A60090, 0x46053034, 0x00000000, 0x45000002,
                0x00000000, 0x46003146, 0xC7A6005C, 0xC7A70024, 0x46073181, 0x3C0840C0,
                0x44883800, 0x00000000, 0x46073180, 0xE7A6008C, 0x46062834, 0x00000000,
                0x45000002, 0x00000000, 0x46003146, 0xC46602D4, 0x460629C1, 0x3C083E99,
                0x3508999A, 0x44881800, 0x00000000, 0x460339C2, 0x46073180, 0xE7A60068,
                0xC46102D0, 0x3C083E19, 0x3508999A, 0x44881800, 0x00000000, 0x46010081,
                0x46031082, 0x46020800, 0xE7A0005C, 0xC7A1005C, 0xC7A20060, 0x46011082,
                0xC46302C0, 0x46021880, 0xE7A20030, 0xC46202C4, 0xC7A30068, 0x46031080,
                0xE7A20034, 0xC7A20064, 0x46011082, 0xC46302C8, 0x46021880, 0xE7A20038,
                0x02A02021, 0x03C02821, 0x27A60020, 0x3C080014, 0x3508C210, 0x01003821,
                0x0C052754, 0x00000000, 0xAFA20098, 0x3C0B0014, 0x356BC210, 0x8D680000,
                0x8D690004, 0x8D6A0008, 0x01094025, 0x010A4025, 0x1100011F, 0x00000000,
                0xC5670000, 0xC7A80030, 0x460839C1, 0x46073A42, 0xC5670004, 0xC7A80034,
                0x460839C1, 0x460739C2, 0x46074A40, 0xC5670008, 0xC7A80038, 0x460839C1,
                0x460739C2, 0x46074A40, 0x3C084780, 0x44884000, 0x00000000, 0x46094034,
                0x00000000, 0x4501010A, 0x00000000, 0x44084800, 0x00000000, 0x3C0A3F80,
                0x010A402A, 0x1500001F, 0x00000000, 0x46090244, 0x00000000, 0x3C0840E0,
                0x44884000, 0x00000000, 0x46084A00, 0x46094203, 0x00000000, 0xC7A70030,
                0xC5610000, 0x460139C1, 0x460839C2, 0x460709C0, 0xE7A70070, 0xC7A70034,
                0xC5610004, 0x460139C1, 0x460839C2, 0x460709C0, 0xE7A70074, 0xC7A70038,
                0xC5610008, 0x460139C1, 0x460839C2, 0x460709C0, 0xE7A70078, 0xAFA0007C,
                0x27A70070, 0x10000002, 0x00000000, 0x27A70030, 0x02A02021, 0x03C02821,
                0x01603021, 0x27A80040, 0xAFA80010, 0x24090001, 0xAFA90014, 0xAFA00018,
                0x0C052754, 0x00000000, 0x8FA30058, 0x044000D8, 0x00000000, 0x00024980,
                0x00025100, 0x012A4821, 0x02A94821, 0xC5240030, 0xC5250034, 0xC5260038,
                0x460421C2, 0x46052A02, 0x460839C0, 0x46063202, 0x460839C0, 0x460701C4,
                0x00000000, 0x3C083F80, 0x44884000, 0x00000000, 0x46074203, 0x00000000,
                0x46082102, 0x46082942, 0x46083182, 0xC7A70060, 0x460439C2, 0xC7A80064,
                0x46064202, 0x460839C0, 0x44083800, 0x00000000, 0x19000004, 0x00000000,
                0x46002107, 0x46002947, 0x46003187, 0xC5270004, 0xC5280014, 0x46083FE8,
                0xC5280024, 0x46083FE8, 0xE7A7009C, 0xC7A70030, 0xC7A80040, 0x460839C1,
                ...
                0x44885800, 0x00000000, 0x460A5AC1, 0x44085800, 0x00000000, 0x1900009A,
                0x00000000, 0xC7A70060, 0x460439C2, 0xC7A80064, 0x46064202, 0x460839C0,
                0xE7A70088, 0xC7A80064, 0x46044202, 0xC7A90060, 0x46064A42, 0x46094301,
                0x46073A02, 0x46052A42, 0x46094200, 0x460C6242, 0x3C083D80, 0x35080000,
                0x44885000, 0x00000000, 0x460A4A42, 0x46094200, 0x46085AC3, 0x00000000,
                0x00000000, 0x46075902, 0x46055942, 0x460C5982, 0x460A3182, 0xC7A0005C,
                0x46040000, 0xC7A20068, 0x46051080, 0xC7A9005C, 0x46093043, 0x00000000,
                0x00000000, 0xC46302D8, 0x460118C0, 0xE46302D8, 0xC46702DC, 0x46071A01,
                0x3C084049, 0x35080FDB, 0x44884800, 0x00000000, 0x3C0840C9, 0x35080FDB,
                0x44885800, 0x00000000, 0x46084834, 0x00000000, 0x45000002, 0x00000000,
                0x460B4201, 0x46004A87, 0x460A4034, 0x00000000, 0x45000002, 0x00000000,
                0x460B4200, 0x3C083ECC, 0x3508CCCD, 0x44885000, 0x00000000, 0x460062C5,
                ...
                0xC7A90088, 0x46093AC2, 0x3C083E00, 0x44886800, 0x00000000, 0x460C58C2,
                0x460018C7, 0x460D18C2, 0x46033180, 0x460018C3, 0xC46902D8, 0x46034A40,
                0xE46902D8, 0x8FA80098, 0x05000006, 0x00000000, 0x3C08BF80, 0x44883800,
                0x00000000, 0x10000005, 0x00000000, 0x3C0842A0, 0x44883800, 0x00000000,
                0x460039C1, 0x44804000, 0x00000000, 0x46083FE8, 0x460739C2, 0x3C083C0C,
                0x3508CCCD, 0x44884000, 0x00000000, 0x460839C2, 0x3C084188, 0x44884000,
                0x00000000, 0x460839C0, 0xC7A8009C, 0xC7A90024, 0x46094201, 0x3C084160,
                0x44884800, 0x00000000, 0x46094201, 0x46083FE9, 0xC7A8008C, 0x46083FE8,
                0xC7A80090, 0x46083FE9, 0xC7A80068, 0x46083941, 0x46003886, 0x3C0B0014,
                0x356BC210, 0xC7A70060, 0xC7A80064, 0x46072042, 0x460830C2, 0x46030840,
                0xC7A30030, 0x46011840, 0xE5610000, 0xC7A30034, 0x460518C0, 0xE5630004,
                0x46082042, 0x460730C2, 0x46030841, 0xC7A30038, 0x46011840, 0xE5610008,
                0x24080001, 0xAD68000C, 0x1000000C, 0x00000000, 0x3C0B0014, 0x356BC210,
                0xC7A70030, 0xE5670000, 0xC7A70034, 0xE5670004, 0xC7A70038, 0xE5670008,
                0xAD60000C, 0xC7A0005C, 0xC7A20068, 0xE46002D0, 0xE46202D4, 0x2402FFFF,
                0x8FBF0050, 0x03E00008, 0x27BD00A0,
            };
            ...
            const float CLIMB_PEAK = 60f;    // height the climb curve reaches at full pull-in (d = 0); the BELL's peak
            float CLIMB_K = (CLIMB_PEAK - REST_H) / (BASE_DIST * BASE_DIST);   // quadratic climb gain — zero slope at touch
            PutVal(143, BASE_DIST, nameof(BASE_DIST));   // resting dist target
            PutVal(434, BASE_DIST, nameof(BASE_DIST));   // reacquisition rest
            PutVal(459, BASE_DIST, nameof(BASE_DIST));   // climb intrusion reference
            PutVal(146, REST_H, nameof(REST_H));   // height target base
            PutVal(472, REST_H, nameof(REST_H));   // climb-curve base
            PutVal(40, CEIL_DIST, nameof(CEIL_DIST));
            PutVal(154, MIN_CEIL_CLEAR, nameof(MIN_CEIL_CLEAR));   // tunnel-duck clearance
            PutVal(479, MIN_CEIL_CLEAR, nameof(MIN_CEIL_CLEAR));   // rim-backstop clearance
            PutVal(167, MIN_GROUND_CLEAR, nameof(MIN_GROUND_CLEAR));
            PutVal(257, SLIDE_MARGIN, nameof(SLIDE_MARGIN));   // proximity-extension reach
            PutVal(353, SLIDE_MARGIN, nameof(SLIDE_MARGIN));   // need standoff
            PutVal(440, SLIDE_GAIN, nameof(SLIDE_GAIN));   // θ reacquisition
            PutVal(117, STICK_SCALE, nameof(STICK_SCALE));
            PutEase(109, 110, STICK_DZ2, nameof(STICK_DZ2));
            PutEase(128, 129, STICK_EASE, nameof(STICK_EASE));
            PutEase(179, 180, HEIGHT_EASE, nameof(HEIGHT_EASE));
            PutEase(187, 188, DIST_EASE, nameof(DIST_EASE));
            PutEase(376, 377, SLIDE_BIAS, nameof(SLIDE_BIAS));
            PutEase(421, 422, SLIDE_FRICTION_INV, nameof(SLIDE_FRICTION_INV));
            PutEase(467, 468, CLIMB_K, nameof(CLIMB_K));
            ...
            WrU32(fs, ElfOff(0x0014C21C), 0x00000000);     // zero-init the contact flag (E_prev.w): 1 = sweep contact last frame -> height-hold
            // ── Decouple vanilla's WALL response: the four EdMoveChara `jal CameraAutoMove` sites (inside the
            //    width-probe-hit branch) SLAM angT with an absolute atan2 re-aim (~90°+/frame observed) when walking
            //    past walls the probes see — 75u/frame eye teleports whose chords cut through buildings faster than
            //    the swept constraint can resolve. Vanilla's wall handling is OUR job now; the normal follow
            //    (stick blocks + the walk re-aim outside these branches) is untouched. jal 0x169B70 = 0x0C05A6DC.
            Guard(0x0016B488, 0x0C05A6DC, "CameraAutoMove call #1"); WrU32(fs, ElfOff(0x0016B488), 0x00000000);
            Guard(0x0016B4BC, 0x0C05A6DC, "CameraAutoMove call #2"); WrU32(fs, ElfOff(0x0016B4BC), 0x00000000);
            Guard(0x0016B4FC, 0x0C05A6DC, "CameraAutoMove call #3"); WrU32(fs, ElfOff(0x0016B4FC), 0x00000000);
            Guard(0x0016B58C, 0x0C05A6DC, "CameraAutoMove call #4"); WrU32(fs, ElfOff(0x0016B58C), 0x00000000);
