...
- **Sun Sword ("Solar Flash")** — Hold guard for a second and a half and the blade whitens to a full charge (the charge-complete pulse marks it); the charge keeps until the next attack. As that swing comes forward the blade returns to normal and the dungeon flashes blinding white, easing back over a second. Every enemy within 300 units takes a light hit (a quarter of the sword's attack, with its element and a melee stagger) and is blinded for five seconds: after its stagger it stops where it stands, raises its guard and holds it (or stands idle if the species has no guard), facing the spot he flashed from. Just before the five seconds are up the guard comes down — through the enemy's own guard-lowering animation — and it stands ready, so you can see it is about to come back at you. While the charge is held Toan himself carries a white tint and a white glow, and the blade's gold brightens to near-white the instant the charge completes.
