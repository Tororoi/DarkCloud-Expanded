...
tools/brownboo/brownboo_rock_hull_builder.py from the _IWA01_* authoring tris + _collar_pairs kept here).
...
Run standalone for a summary: python3 tools/brownboo/brownboo_camera_collision.py
"""
...
def _iwa01_build():
    ...
    CSG difference), top & bottom caps stripped. Authored in tools/brownboo/brownboo_rock_hull_builder.py and FROZEN to
    tools/brownboo/brownboo_rock_hull_data.py as a plain tri literal, so the bake needs no CSG deps. Re-run that generator
