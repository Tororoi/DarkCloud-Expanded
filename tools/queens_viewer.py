...
# ── REVERTED h04/05/08/09 custom `_c` overlay (former queens_building_camera_hulls.py) — kept as a
# viewer layer only; the bake was dropped 2026-08 ("just use the vanilla _c meshes").
...
def _hcam_node_tris(scn, DIR, sub_name):
...
def _hcam_candidates():
    ...
    for part in CAND_PARTS:
        ...
        nodes = _hcam_node_tris(scn, DIR, part)
...
for _bn, _ct in _hcam_candidates().items():
    ...
    print(f"  _c candidate {_bn}: {len(_ct)} tris in {len(kd_split(_ct, 200, axes=(0, 2)))} bake nodes")
