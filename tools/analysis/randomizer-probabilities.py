...
ROOT = "."   # repo-root-relative; run this from the repo root: python3 tools/analysis/randomizer-probabilities.py
...
L.append(f"_Generated {datetime.date.today().isoformat()} from `Dark Cloud Improved Version/EnemyData.cs` + `EnemyModelInjector.cs` by Monte-Carlo simulation ({N:,} floors per dungeon). Re-run `tools/analysis/randomizer-probabilities.py` after changing themes/membership._\n")
