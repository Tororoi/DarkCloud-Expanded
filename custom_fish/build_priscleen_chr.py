...
def dc1_info_cfg(dc2_cfg):
    ...
    # ANIMATION: DC1 skeletal skinning (bones via .mot, vertex weights via .wgt) — the .mot/.wgt formats &
    # all frame indices are DC1-compatible (verified offline). The ONE structural difference from f01a
    # (which flaps fine) is that f01a has exactly ONE morphable/skinned frame (obj9) while DC2 Priscleen
    # names TWO (skin1=224v body, skin2=56v). Double-buffering BOTH (ALLOC_DBUFF skin1+skin2) crashed PCSX2
    # ~4s into the angle display. DBUFF_SKINS controls which morphable frames we build: ['skin1'] animates
    # just the body (matches f01a's single-part shape — the flap test); [] = static/stable; ['skin1',
    # 'skin2'] = full (crashed). See the dc2-archive-and-priscleen memo.
    DBUFF_SKINS = ['skin1']
    lines = [
        f'IMG 0,"{img}"', 'IMG_END', 'MATERIAL_ANIME 0', 'VERTEX_ANIME 1', '',
        *[f'ALLOC_DBUFF "{s}"' for s in DBUFF_SKINS], f'MODEL "{model}"', f'BODY_SIZE {bx},{by},{bz}', '',
        f'MOTION 0, "{m_mot}", "{m_bbp}", "{m_wgt}"', 'SHADOW_MOTION "", "", ""', 'KEY_START 0',
    ]
    # DC1 fish use SEVEN motions (SetMotion 0-6); the caught/held fish plays motion 5 (釣り上げられた).
    # Priscleen (DC2) ships only THREE: 0=通常(normal), 1=バトル(battle), 2=釣れた時(caught). If the game
    # requests a motion index >= 3, DC1 reads PAST the KEY table into garbage → a bogus .mot frame range →
    # on a skinned mesh that garbage becomes NaN vertices → CRASH (a static mesh survives because nothing
    # applies it). So map DC1's 7 slots onto Priscleen's 3 by MEANING, making every index in-range and
    # routing motion 5 (the held-fish "caught" pose) to Priscleen's 釣れた時 range 40-80 — the flap we want.
    K = [(s, e, w) for (s, e, w) in keys] or [('4','14','0.35')]
    SLOT = [0, 1, 1, 2, 2, 2, 0]     # DC1 motion i -> Priscleen source motion (0/1/2)
    for i, si in enumerate(SLOT):
        s, e, w = K[si] if si < len(K) else K[-1]
        lines.append(f'KEY\t{s},\t{e},\t{w},\t//dc1-motion-{i}')
    lines += ['MOTION_END', '']
