...
- [ ] **Sun Sword (Solar Flash)** — Hold guard ~1.5 s: blade ramps to white, charge pulse; next swing flashes the dungeon white (eases back ~1 s); enemies within 300 u take a light hit, then stagger, stop where they stand, play their guard raise and hold the loop (or idle if the species has none), facing Toan's flash spot for 5 s (no freeze status); floor change / weapon swap clears everything; blade whitens only after repatching the ISO
