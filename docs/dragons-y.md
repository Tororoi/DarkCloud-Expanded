...
`Weapons/Xiao/DragonsY.cs` (thread `CustomXiaoEffects.DragonsYEffect`), `Weapons/Xiao/GemronShots.cs`.
...
switched-to element resident. The rewind assumes nothing else took monster-pool memory after our entry mid-floor. `GemronShots.Start` (app start) keeps the block matched to Xiao's equipped inventory weapon — Dragon's Y with an element
